/*
	version 2.0, 不支持各个 js css　文件可能需要的属性, 例如 async mode 等,
	将在 3.0 中解决
*/
 ...
 * 注意: 使用该加载器加载的脚本, 全部被默认为异步(MSN: 从脚本中创建的脚本默认为异步 async)
 * @deprecated 已废弃. 局限性比较大, 主要是只能加载 css/js 文件, 不能配置任何属性, 
 * 例如 js 的 defer, css 的 media等. 
 * 为了适应这些需求, 请使用它的升级版: page-resource-loader1.0 
 * */
class JscssLoader {
	...
	private async _onWindowLoaded_() {
		...
		for (let i = 0; i < ele_jss.length; i++) {
			...
			if (idx != -1) {
				...
				// 由于全局的json配置文件位置相对于 jscss-loader.t(j)s, 所以有必要先获取
				// jscss-loader.t(j)s 在实际的 html 中的位置
				this._globalResDefault = `${src.slice(0, idx)}${this._globalResDefault}`;
				this._loadedFilePaths.push(src);
				const node = ele_jss[i];
				this._loadedJsnodes.push(node); //记录好,以备最后在main方法中移除
