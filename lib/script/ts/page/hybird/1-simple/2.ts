...
function windowLoadedHandler(evt?: Event) {
	console.log('2.ts: ready !');
	commonFun('congratulations');
	document.querySelector('.test .two')?.addEventListener('click', function (this: HTMLButtonElement, evt: Event) {
...
function testObjectExtension() {
	...
	[obj, dt, nl, rg, fun, str, window].forEach(v => {
		//v?.deepFreeze();
		//console.log(v, v?.deepClone());
	});

	//str.deepFreeze();

	//const obj2 = obj.deepClone();
	//@ts-ignore
	// obj2.d = /^pity[abc]*$/gi; obj2.b.h[3] = new Date(1999, 5, 21, 8, 15, 2, 192);
	// console.log(obj, obj2);
	// obj2.compareTo(obj, "root");

	const defs: Array<PropertyDeference> = [];
	const windowClone = window.deepClone();
	console.log(window, windowClone);
	//windowClone.compareTo(window, "window");
