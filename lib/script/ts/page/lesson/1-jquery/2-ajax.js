...
JscssLoader.getInstance().startEntry({
    ...
    main: function () {
        var remote_domain = "https://localhost:7018/";
        base_demo();
        //   check_datatype_demo();
        //contentType_demo();
        //serialize_form_demo();
        ...
        function base_demo() {
            $('.base_demo input[type="button"]').click(function () {
                ...
                var jresult = $('div.ajax-result').empty();
                var jtitle = $('<h1>').appendTo(jresult);
                var jcontent = $('<h5>').appendTo(jresult);
                var jtail = $('<addr>').appendTo(jresult);
                $.ajax({
                    async: true,
                    cache: true,
