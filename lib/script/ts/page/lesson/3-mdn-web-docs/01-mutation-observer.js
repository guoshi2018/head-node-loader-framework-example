    ...
    main: function () {
        ...
        function demo1() {
            ...
            if (null != list) {
                ...
                var callback_1 = function (mutations, observer) {
                    ...
                    mutations.forEach(function (mutation) {
                        ...
                        if (mutation.type === "childList") {
                            if (mutation.target && mutation.addedNodes.length) {
                                console.log("At least one child node ".concat(mutation.target, " has been added!"), mutation.target);
                            }
                            if (mutation.target && mutation.removedNodes.length) {
                                console.log("At least one child node ".concat(mutation.target, " has been removed!"), mutation.target);
                            }
                            // do somwthings
                            var list_values = new Array();
                            for (var i = 0; i < list.children.length; i++) {
                                var ele = list.children.item(i);
                                if (null != ele) {
                        ...
                        if (mutation.type === "attributes") {
                            ...
                            var _a = list.style, left = _a.left, top_1 = _a.top, width = _a.width, height = _a.height;
                            var style = {
                                left: left,
                                top: top_1,
                                width: width,
                ...
                observer_1.observe(list, config);
                ...
                setTimeout(function () {
                    var restMutations = observer_1.takeRecords();
                    if (restMutations.length > 0) {
                        console.log('some changes are not be handled.....');
                        callback_1(restMutations, observer_1);
                    }
                    else {
