...
JscssLoader.getInstance().startEntry({
    ...
    //undefined, null 或 空字符串 '' 或 输入路劲不存在, 则放弃公共先决资源文件的加载
    //globalRes: undefined, //这也表示取消
    //是否启用调试
    debug: true,
    ...
    privateRes: [
        [
            //切换主题,注意最后一个按钮
            //'/lib/external-core/jquery-ui-1.13.1.custom/ui-themes/le-frog/jquery-ui.min.css',
            //'/lib/external-core/jquery-ui-1.13.1.custom/ui-themes/smoothness/jquery-ui.min.css',
            //'/lib/external-core/jquery-ui-1.13.1.custom/ui-themes/sunny/jquery-ui.min.css',
            //'/lib/external-core/jquery-ui-1.13.1.custom/ui-themes/humanity/jquery-ui.min.css',
    ...
    main: function () {
        ...
        //i 表示使用第一种方法还是第二种方法
        function fillData(i) {
            var jtbody = $('table#inventory tbody');
            //产品总数
            var quantity = $('td:nth-child(2)', jtbody)
                .map(function (idx, ele) {
                    return $(ele).text();
                }).get(); //等价于 toArray()
            $('tr#sum td:nth-child(2)').text(eval("$.gx_sum".concat(i))(quantity).toFixed(2));
            $('tr#average td:nth-child(2)').text(eval("$.gx_avg".concat(i))(quantity).toFixed(2));
        }
        //第三种方法,尽量减少$命名空间的污染
        function fillData3() {
            var jtbody = $('table#inventory tbody');
        ...
        function configPluginTestingButton() {
            var jplugin = $('table,h1', '#plugin');
            $('input[type="button"].err').click(function () {
                $('tr').swapClass1('one', 'two');
            });
            $('input[type="button"].suc').click(function () {
                $('tr').swapClass2('one', 'two');
            });
            $('input[type="button"].shadow1').click(function () {
                jplugin.shadow1();
            });
            $('input[type="button"].shadow2').click(function () {
                jplugin.shadow2({
                    copies: 8,
                    opacity: 0.24
                });
            });
            $('input[type="button"].shadow3').click(function () {
                //使用默认
                //$('table,h1').shadow3()
                //逐个修改默认选项的属性
        ...
        function configWidgetTestButton() {
            var jlink = $('a', 'table tbody tr td');
            //创建
            $('input[type="button"].create', 'div.container#widget').click(function () {
                //无参创建,采用默认
                //jlink.simpleTooltip();
                //同上
                //jlink.simpleTooltip({});
                ...
                jlink.simpleTooltip({
                    ...
                    content: function () {
                        ...
                        return "".concat($(this).data('ttt'), ",").concat($(this).text());
                    }
                });
            });
            //启用
            $('input[type="button"].enable', 'div.container#widget').click(function () {
                jlink.simpleTooltip('enable');
            });
            //禁用
            $('input[type="button"].disable', 'div.container#widget').click(function () {
                jlink.simpleTooltip('disable');
            });
            //打开
            $('input[type="button"].open', 'div.container#widget').click(function () {
                jlink.simpleTooltip('open');
            });
            //关闭
            $('input[type="button"].close-tip', 'div.container#widget').click(function () {
                jlink.simpleTooltip('close');
            });
            //方法连缀
            $('input[type="button"].cluster', 'div.container#widget').click(function () {
                jlink.simpleTooltip('test1').simpleTooltip('test2').simpleTooltip('test1');
            });
            //销毁
            $('input[type="button"].destroy', 'div.container#widget').click(function () {
                //console.log('current alive:', jlink.simpleTooltip('alive'));
                jlink.simpleTooltip('destroy');
            });
            $('h2.disabled').click(function (evt) {
                var result;
                try {
            ...
            jlink.on('simpleTooltipopen', function () {
