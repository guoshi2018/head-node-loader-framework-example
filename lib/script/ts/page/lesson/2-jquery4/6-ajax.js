...
JscssLoader.getInstance().startEntry({
    ...
    main: function () {
        ...
        function configAjaxProcess() {
            var jloading = $('<div id="loading">Loading...</div>')
                .insertBefore(jdict);
        ...
        function configMockButtons() {
            //使用html片段a.html填充
            $('#letter-a .button').click(function (evt) {
            ...
            $('#letter-b .button').click(function () {
                jdict.empty();
                $.getJSON('/page/lesson/2-jquery4/6-server-side/b.json', function (data) {
                    // console.log(data);
            ...
            $('#letter-e a').click(function (evt) {
                evt.preventDefault();
                var server_page = '/page/lesson/2-jquery4/6-server-side/e.php';
                //const server_page = '/page/lesson/2-jquery4/6-server-side/ee.php'; // test server error
                var reqData = {
                    term: $(this).text()
                };
                //get: ok
                $.get(server_page, reqData, function (data) {
                    jdict.html(data);
                }).fail(function (jqXHR) {
            ...
            $('#letter-f form').submit(function (evt) {
                evt.preventDefault();
                var server_page = '/page/lesson/2-jquery4/6-server-side/f.php';
                //使用常规的term搜索: OK
                // $.get(server_page, {
                ...
                $.get(server_page, formValues, function (data) {
                    jdict.html(data);
                });
            });
        }
