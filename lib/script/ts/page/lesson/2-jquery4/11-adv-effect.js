    ...
    main: function () {
        ...
        function configHoverImage() {
            ...
            function switchByEvent(jele, evt) {
                var _a = correctParams(evt.type), size = _a[0], padding = _a[1];
                jele.animate({
                    width: size,
                    height: size,
                    padding: padding
                ...
                return __awaiter(_this, void 0, void 0, function () {
                    return __generator(this, function (_a) {
                        switch (_a.label) {
                            case 0:
                                console.log("------------------------------------------");
                                console.log("The mouse has stopped active for ".concat(idleBysecond, " second(s),") +
                                    "and the page will close after ".concat(warnBysecond, " second(s)"));
                                return [4 /*yield*/, Guoshi.Misc.sleep(warnBysecond * 1000)];
                            case 1:
                                _a.sent();
                                console.log("now window is closed ".concat(++close_times));
                                return [2 /*return*/];
                        }
                    });
                });
        ...
        function configClickPhotoToToShowDetailsThenBio() {
            //#region 准备动态显示的人物简介
            var jmovable = $('<div id="movable"></div>').appendTo('body'); //动态容纳不同简介
            var bioBaseStyle = {
                display: 'none',
                height: '5px',
                width: '25px'
             ...
             */
            function showBio() {
                var jbio = $(this).siblings('p.bio');
                var jparent = jbio.parent();
                var parent_loc = jparent.offset() || {
                    left: 0,
                    top: 0
                };
                var par_border_top = parseFloat(jparent.css('border-top-width').replace(/s+|px/gi, ''));
                var par_margin_top = parseFloat(jparent.css('margin-top').replace(/s+|px/gi, ''));
                var startStyle = $.extend(bioBaseStyle, parent_loc); //混入容器的left,top坐标
                var endStyle = {
                    width: jbio.width(),
