...
JscssLoader.getInstance().startEntry({
    ...
    main: function () {
         ...
         */
        var Sort_Key = 'sort-key';
        /**
         * 持久存在于各行的data-属性里的, 由所有可能使用的排序列名称作为其属性名的
         * 对象, 各个属性的值在选择排序前已经配置好
         */
        var Sort_Key_Object = 'sort-key-object';
        var Sort_Key_Array = 'sort-key-array';
        /**
        ...
        var SortKeyMethods = {
             ...
             */
            alpha: function (jele) {
                var key = jele.find("span.".concat(Sort_Key)).text();
                var sep = key == '' ? '' : ' ';
                key += sep + jele.text();
             ...
             */
            numeric: function (jele) {
                var num = jele.text().replace(/^[^\d.]*/, '');
             ...
             */
            date: function (jele) {
                var key = Date.parse('1 ' + jele.text());
                return key;
            }
        };
        //#endregion
        ...
        function configTableOneSortable1() {
            ...
            var jtable = $('table#t-1');
            var jheaders = jtable.find('thead th').slice(1);
            ...
            jheaders.on('click', function (evt) {
                ...
                var rows = jtable.find('tbody > tr').get();
        ...
        function configTableOneSortable2() {
            ...
            jheaders.on('click', function (evt) {
                ...
                //元素数组排序,然后append至容器,append不会导致增加副本,只是移动
                var column = $(this).index();
                var rows = jtable.find('tbody > tr').each(function () {
                    var key = $(this).children('td').eq(column).text();
                    $(this).data(Sort_Key, key.trim().toUpperCase());
                }).get();
                rows.sort(function (a, b) {
                    var keyA = $(a).data(Sort_Key);
                    var keyB = $(b).data(Sort_Key);
                    return keyA > keyB ? 1 : (keyA == keyB ? 0 : -1);
                });
                rows.forEach(function (row) {
                    jtable.children('tbody').append(row);
        ...
        function configTableOneSortable3() {
            ...
            jheaders.on('click', function (evt) {
                ...
                var rows = jtable.find('tbody > tr').each(function () {
                    var jtr = $(this);
                    var jtd = jtr.children('td').eq(column);
                    var key = jtd.find("span.".concat(Sort_Key)).text();
                    var sep = key == '' ? '' : ' ';
        ...
        function configTableOneSortable4() {
            ...
            jheaders.on('click', function (evt) {
                ...
                //元素数组排序,然后append至容器,append不会导致增加副本,只是移动
                var jheader = $(this);
                var column = jheader.index();
                var keyType = jheader.data(Key_Type);
        ...
        function configTableOneSortableByMyself1() {
            ...
            //#endregion
            //确定属性名(使用各个cell的text)
            var props = ['']; //第一个(index=0)属性名不被采用
            jheaders.each(function (idx, td) {
                props.push(td.innerText);
            });
            //#region 预先存入每行的sort-key-object对象内
            jtable.find('tbody tr').each(function (idx, tr) {
                var jtr = $(tr);
                var keys = {};
                jtr.children('td').each(function (colIdx, td) {
                    var jtd = $(td);
                    //姓氏优先的兼容处理
                    var v = jtd.find("span.".concat(Sort_Key)).text();
                    var sep = v == '' ? '' : ' ';
            ...
            jheaders.on('click', function (evt) {
                ...
                //元素数组排序,然后append至容器,append不会导致增加副本,只是移动
                var column = jtd.index();
                var rows = jtable.find('tbody > tr').get();
                rows.sort(function (a, b) {
                    //console.log($(a).data(Sort_Key_Object), $(b).data(Sort_Key_Object));
                    var keyA = $(a).data(Sort_Key_Object)[props[column]];
                    var keyB = $(b).data(Sort_Key_Object)[props[column]];
                    var rst = keyA > keyB ? 1 : (keyA == keyB ? 0 : -1);
                    return rst * sortAsc;
                });
                rows.forEach(function (row) {
                    jtable.children('tbody').append(row);
        ...
        function configTableOneSortableByMyself2() {
            ...
            jheaders
                ...
                .each(function (idx, td) {
                    var method = SortKeyMethods[td.className.replace(/^sort-/, '')];
            ...
            jtable.find('tbody tr').each(function (idx, tr) {
                var jtr = $(tr);
                var keys = [];
                jtr.children('td').slice(1).each(function (colIdx, td) {
            ...
            jheaders.on('click', function (evt) {
                ...
                //元素数组排序,然后append至容器,append不会导致增加副本,只是移动
                var column = jtd.index() - 1; //jtd.index()获取到的是在所有兄弟中的排行.
                var rows = jtable.find('tbody > tr').get();
                rows.sort(function (a, b) {
                    //console.log($(a).data(Sort_Key_Object), $(b).data(Sort_Key_Object));
                    var keyA = $(a).data(Sort_Key_Array)[column];
                    var keyB = $(b).data(Sort_Key_Array)[column];
                    var rst = keyA > keyB ? 1 : (keyA == keyB ? 0 : -1);
                    return rst * sortAsc;
                });
                rows.forEach(function (row) {
        ...
        function configTableThreeSortable() {
            var imgBaseUrl = '/lib/image/jpg/12-adv-dom-oper';
            /**
             * 构造一个tr的outer html
             * @param row
             */
            function buildRow(row) {
                var authors = [];
                row.authors.forEach(function (name) {
                    authors.push(name.first_name + ' ' + name.last_name);
                });
                var html = "\n\t\t\t\t<tr>\n\t\t\t\t\t<td><img src=\"".concat(imgBaseUrl, "/").concat(row.img, "\"/></td>\n\t\t\t\t\t<td>").concat(row.title, "</td>\n\t\t\t\t\t<td>").concat(authors.join(', '), "</td>\n\t\t\t\t\t<td>").concat(row.published, "</td>\n\t\t\t\t\t<td>").concat(row.price, "</td>\n\t\t\t\t</tr>\n\t\t\t\t");
                return html;
            }
            /**
            ...
            function buildRows(rows) {
                var trs = rows.map(buildRow);
                return trs.join('');
            }
            $.getJSON('/lib/script/json/12-adv-dom-oper/books.json', function (data) {
                var jtable3 = $('table#t-3');
        ...
        function createElementTersely() {
            $('table').each(function (idx) {
                ...
                $('<h3></h3>', {
                    ...
                    "class": 'table-title',
                    text: "This is h3 caption for table".concat(idx + 1),
                    data: {
                        index: idx,
                        a: 18,
                        b: 'good'
