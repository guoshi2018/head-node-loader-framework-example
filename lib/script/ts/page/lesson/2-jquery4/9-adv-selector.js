...
JscssLoader.getInstance().startEntry({
    ...
    main: function () {
        ...
        function configHyperlink() {
            var jlink = $('#topics a').click(function (evt) {
                //prepare:
                var jele = $(this);
                var topic = jele.text();
                $('#news tr').show();
        ...
        function showJqueryObjectProperties() {
            var jele1 = $('#release'); //prevObject == undefined
            var jele2 = $('#release', '#topics'); //prevOjbect == $('div#topics'),虽然两者无关
            var jele3 = jele1.nextAll();
            threeProp(jele1, 'jele1');
            threeProp(jele2, 'jele2');
            threeProp(jele3, 'jele3');
            function threeProp(jq, name) {
                console.log("".concat(name, " length:").concat(jq.length, ", props:"), {
                    context: jq.context, selector: jq.selector, prevObject: jq.prevObject
                });
                if (jq.prevObject) {
                    console.log("prevObject html of ".concat(name, ":").concat(jq.prevObject.html()));
                }
            }
        }
        /**
        ...
        function testCustomGroupSelector(num) {
            $('#news tr').filter(":group(".concat(num, ")")).addClass('alt');
        }
