    ...
    main: function () {
        ...
        function configHoverImage2() {
            ...
            $('div#gallery').on('mouseenter mouseleave', '.photo', function (evt) {
                var jdetails = $('.details', this); //注意和上面一模一样
                evt.type.toLowerCase() == 'mouseenter' ?
                    jdetails.fadeTo('fast', 0.5) :
                    jdetails.fadeOut('fast');
            });
        ...
        function configLoadmoreByHyperlink() {
            ...
            $('a#more-photos').click(function (evt, scrollToVisible) {
                ...
                if (nextPage <= MAX_PAGE) {
                    $.get("/lib/html-clip/".concat(nextPage, ".html"), function (data) {
                        var _a;
                        //$('#gallery').append(data);
                        var jdata = $(data).appendTo('#gallery');
                        if (scrollToVisible) {
                            $(window).scrollTop(((_a = jdata.offset()) === null || _a === void 0 ? void 0 : _a.top) || 0);
                            //	$(window).scrollTop(10);
                        }
                    });
                    if (nextPage == MAX_PAGE) {
        ...
        function configLoadmoreByScrollbar() {
            var scroll = 0; //记录滚动条滚动次数
            var bott = 0; //记录滚动条到底次数
            //4 配置滚动调试件,当滚至文档底部, 触发document的loadmore事件
            $(document).on('scroll.for-loadmore', function (evt) {
                console.log('scroll bar:', ++scroll);
                if (nextPage < MAX_PAGE) {
                    //文档顶部到窗口底部的距离: 
                    var distance = ($(window).scrollTop() || 0) + ($(window).height() || 0);
                    var h = $('#container').height() || 0;
                    console.log("doctop-winbot:".concat(distance.toFixed(0), ", h:").concat(h.toFixed(0)));
                    if (distance >= h) { //见 checkScrollPosition 注释
                        console.log("to bottom ".concat(++bott));
        ...
        function configLoadmoreByMousewheel() {
            var wheel = 0; //记录滚轮在窗体未出现滚动条时,向下滚动的次数
            //5 配置鼠标滚轮事件: 滚动条未出现时, 鼠标滚轮向下滚动, 触发document的loadmore事件,
            // 出现滚动条, 则取消该事件响应
        ...
        function testAjax() {
            return __awaiter(this, void 0, void 0, function () {
                ...
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0: return [4 /*yield*/, $.get('/lib/html-clip/1.html')];
                        case 1:
                            data = _a.sent();
                            console.log(data);
                            return [2 /*return*/];
        ...
        function testScrollBarMethod() {
            $('div.footer').click(function () {
                ...
                var sbd = 0;
                $(window).on('scroll', function () {
                    var win_h = $(window).height() || 0; //窗口高度
                    var src_loc = $(window).scrollTop() || 0;
                    var doc_h = $(document).height() || 0;
            ...
            function examScroll(evt) {
                return __awaiter(this, void 0, void 0, function () {
                    var jele, origH, h, resp;
                    return __generator(this, function (_a) {
                        switch (_a.label) {
                            case 0:
                                jele = $(evt.target);
                                ...
                                return [4 /*yield*/, Guoshi.Misc.sleep(3258)];
                            case 1:
                                resp = _a.sent();
        ...
        function testTrigger() {
            ...
            $('div.trigger-test')
                ...
                .on('hello', function (evt) {
                    console.log('div hello:', ++divHello);
                });
            $('a.baidu,a.sina').on('click', function (evt) {
                console.log("".concat(evt.target.innerHTML, " clicked."));
            });
            $('a.baidu,a.sina').on('hello', function (evt) {
                console.log("".concat(evt.target.innerHTML, " hello."));
            });
            //mock click
            $('input.trigger-click').on('click', function (evt) {
                console.log('mock click use trigger:---------------------');
                $('a.baidu,a.sina').trigger('click');
            });
            $('input.triggerHandler-click').on('click', function (evt) {
                console.log('mock click use triggerHandler:-----------------');
                $('a.baidu,a.sina').triggerHandler('click');
            });
            //mock hello
            $('input.trigger-hello').on('click', function (evt) {
                console.log('mock hello use trigger:---------------------');
                $('a.baidu,a.sina').trigger('hello');
            });
            $('input.triggerHandler-hello').on('click', function (evt) {
                console.log('mock click use triggerHandler:-----------------');
        ...
        function testExtEvent1() {
            var jouter = $('div.ext-event');
            var jmiddle = $('div.middle');
            //直接
            ...
            //代理
            jouter.on('payoff', 'div.middle', function (evt) {
                console.log('[outer delegate payoff] middle receive: ', evt.type);
            });
            jouter.on('bind-payoff', 'div.middle', function (evt) {
                console.log('[outer delegate bind-payoff] middle receive: ', evt.type);
            });
            jouter.on('delegate-payoff', 'div.middle', function (evt) {
        ...
        function chapterExercise() {
            ...
            //2
            $(window).on('pageNext', function (evt, pageIndex) {
                console.log("page ".concat(pageIndex, " loaded successfully!"));
            });
            //3.应放在起始位置最佳
            $('div#loadingMsg')
                .ajaxStart(function () {
            ...
            (function () {
                function printMouseLocation(ele, evt) {
                    var loc = Guoshi.Html.relLocFrom(ele, evt);
                    console.log("mouse x:".concat(loc.left, ",y:").concat(loc.top));
                }
                //4. 普通方法, 非节流
