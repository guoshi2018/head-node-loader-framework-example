...
JscssLoader.getInstance().startEntry({
    ...
    main: function () {
        ...
        function configSwitcherButtons() {
            var jspeech = $('div.speech');
            var jspanCurSize = $('span#curSize');
            var defaultFontSize = parseFloat(jspeech.css('font-size'));
            jspanCurSize.text(defaultFontSize.toFixed(2) + 'px');
            $('#switcher button').click(function () {
                var num = parseFloat(jspeech.css('font-size')); //fontSize ok too.
                var flag = this.id.split('-')[1];
                switch (flag) {
                    case 'large':
        ...
        function configMockButtons() {
            ...
            $('#mock-slideToggle').click(function (evt) {
                var jp1 = $('p').eq(1);
                jp1.animate({
                    height: 'toggle',
                    margin: 'toggle',
            ...
            $('#anim-switcher-box').click(function (evt) {
                var pw = $('div.speech p').outerWidth();
                var jsw = $('div#switcher');
                var sww = jsw.outerWidth();
            ...
            $('#anim-queue-switcher-box').click(function (evt) {
                var pw = $('div.speech p').outerWidth();
                var jsw = $('div#switcher');
                var sww = jsw.outerWidth();
            ...
            $('#anim-opacity-move-at-the-same-time').click(function (evt) {
                var pw = $('div.speech p').outerWidth();
                var jsw = $('div#switcher');
                var sww = jsw.outerWidth();
                jsw.css('position', 'relative')
            ...
            $('#mult-ele-anim').click(function (evt) {
            ...
            $('#mult-ele-anim-step').click(function (evt) {
                var jclickedItem = $('p').eq(2).css('border', '1px solid #333')
                    .one('click', function (evt) {
            ...
            $('#mult-ele-by-queue').click(function (evt) {
                $('p').eq(2).css('border', '1px solid #333')
                    .one('click', function (evt) {
                        $(evt.target).next().slideDown(3000)
                            .queue(function () {
