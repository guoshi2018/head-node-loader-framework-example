...
JscssLoader.getInstance().startEntry({
    ...
    main: function () {
        ...
        function runJqueryCookie() {
            ...
            var rst1 = $.cookie('ck 1', 'hello+_yes');
            var rst2 = $.cookie('ck-2', 'world');
            var rst3 = $.cookie('ck 3', 'sit down', {
                expires: 8,
                path: '/',
                secure: false
            });
        ...
        function runBook() {
            ...
            function configBooks() {
                var imgBaseUrl = '/lib/image/jpg/7-use-plugin';
                var books = [{
                    filename: '(01).jpg',
                ...
                */
                var jsecBook = $('<section class="books"/>')
                    .appendTo('body');
                var jul = $('<ul id="books"/>').appendTo(jsecBook);
                books.forEach(function (book) {
                    $("\n\t\t\t\t\t<li>\n\t\t\t\t\t\t<div class=\"item\">\n\t\t\t\t\t\t\t<img src=\"".concat(imgBaseUrl, "/").concat(book.filename, "\" alt=\"").concat(book.title, "\" title=\"").concat(book.title, "\" />\n\t\t\t\t\t\t\t<div class=\"info\">\n\t\t\t\t\t\t\t\t<div class=\"title\">").concat(book.title, "</div>\n\t\t\t\t\t\t\t\t<div class=\"author\">").concat(book.author, "</div>\t\t\t\t\t\t\n\t\t\t\t\t\t\t</div>\t\t\t\t\t\t\n\t\t\t\t\t\t</div>\n\t\t\t\t\t</li>\n\t\t\t\t\t")).appendTo(jul);
                });
                //暂停/恢复切换按钮
