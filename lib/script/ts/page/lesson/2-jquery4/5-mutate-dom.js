...
JscssLoader.getInstance().startEntry({
    ...
    main: function () {
        ...
        function configMockButtons() {
            ...
            $('#use-attr').click(function () {
                $('div.chapter a[href*=wikipedia]').attr({
                    //attr方法,不论何种属性, 均出现在html中
                    rel: 'external',
                    title: function (index, oldTitle) {
                        return "Learn more about ".concat($(this).text(), " at Wikipedia");
                    },
                    id: function (index, oldId) { return "wikilink-".concat(index); },
                    three: 'three-value',
                    four: 'four-value'
                });
            });
            //<input type="button" id="use-prop" value="prop方法操作属性"/>
            $('#use-prop').click(function () {
            ...
            $('#view-result').click(function () {
                var arr_name = ['one', 'two', 'three', 'four', 'type', 'rel', 'id', 'title'];
                var anch = $('div.chapter a[href*=wikipedia]')[2];
                var jres = $('#result');
                jres.val('使用 domElement[prop-name] 获取\n');
                for (var p in anch) {
                    if (arr_name.includes(p)) {
                        //@ts-ignore            
                        jres.val(jres.val() + "".concat(p, ":").concat(anch[p], "\n"));
                    }
                }
                ...
                for (var p in anch) {
                    if (arr_name.includes(p)) {
                        //@ts-ignore            
                        jres.val(jres.val() + "".concat(p, ":").concat(anch.attributes[p], "\n"));
                    }
                }
                jres.val(jres.val() + '\n使用 $(...).attr() 获取\n');
                for (var p in anch) {
                    if (arr_name.includes(p)) {
                        //@ts-ignore            
                        jres.val(jres.val() + "".concat(p, ":").concat($(anch).attr(p), "\n"));
                    }
                }
                jres.val(jres.val() + '\n使用 $(...).prop() 获取\n');
                for (var p in anch) {
                    if (arr_name.includes(p)) {
                        //@ts-ignore            
                        jres.val(jres.val() + "".concat(p, ":").concat($(anch).prop(p), "\n"));
                    }
                }
            ...
            $('#create-top-anchor').one('click', function () {
            ...
            $('#transfer-footnote1').one('click', function (evt) {
                //简单的提取脚注到footer前
                ...
                $('span.footnote')
                    .each(function (idx, ele) {
                        var t = $(ele).text();
                        var n = idx + 1;
                        $(ele).text("".concat(n, ".").concat(t))
                            .before("<a id=\"ref-".concat(n, "\" \n              class=\"text-reference\" href=\"#footnote-").concat(n, "\"><sup>").concat(n, "</sup></a>"));
                    })
                    .insertBefore('div#footer')
                    .css('text-indent', '2em')
                    .wrap(function (idx) {
                        var n = idx + 1;
                        return "<a id=\"footnote-".concat(n, "\" href=\"#ref-").concat(n, "\"/>");
                    });
            });
            //<input value="转移脚注到footer之前,使用ol有序列表自动生成"/>
            //这是书上原版P94
            $('#transfer-footnote2').one('click', function (evt) {
                //简单的提取脚注到footer前
                //$('span.footnote').insertBefore('div#footer');
                $('span.footnote')
                    .insertBefore('div#footer')
            ...
            //这是书上原版P95
            $('#transfer-footnote3').one('click', function (evt) {
                var jnotes = $('<ol id="notes"></ol>').insertBefore('#footer');
                $('span.footnote').each(function (idx, ele) {
                    $("<sup>".concat(idx + 1, "</sup>")).insertBefore(ele);
                    $(ele).appendTo(jnotes).wrap('<li></li>');
                });
            });
            //<input value="使用反向插入方法实现链式方法调用"/>
            //书上原版P96
            $('#transfer-footnote4').one('click', function (evt) {
                var jnotes = $('<ol id="notes"></ol>').insertBefore('#footer');
                $('span.footnote').each(function (idx, ele) {
                    var n = idx + 1;
                    $(ele)
                        .before("\n              <a href=\"#footnote-".concat(n, "\" id=\"context-").concat(n, "\" class=\"context\">\n                <sup>").concat(idx + 1, "</sup>\n              </a>\n            "))
                        .appendTo(jnotes)
                        .append("\n              &nbsp;<a href=\"#context-".concat(n, "\">context</a>\n            "))
                        .wrap("\n            <li id=\"footnote-".concat(n, "\"/>\n            "));
                });
            });
            //<input value="按钮到页脚,但无事件响应" />
            $('#clone-elements-without-event').one('click', function (evt) {
                $('div.t-btns').clone(true, true).appendTo('body');
            });
            //<input type="button" id="clone-to-extrusive-reference" value="通过复制创建突出引用"/>
            $('#clone-to-extrusive-reference').one('click', function (evt) {
                $('span.pull-quote').each(function (idx) {
                    var jpar = $(this).parent('p');
                    jpar.css('position', 'relative');
                    var jcopy = $(this).clone();
                    jcopy
                        .addClass('pulled')
                        .prependTo(jpar)
                        .find('span.drop')
